...
export const create = async (req, res) => {
  try {
    ...
    switch(true){
      ...
      case !category.trim():
        return res.json({error:"Category in Required"})
      ...
      // Saving ProductData into Database
      product.save();
      res.json(product);
  } catch (err) {
    console.log(err);
...
export const list  = async (req,res)=>{
  try{
   const products =  await Product.find({})
    ...
    .limit(12) // limit page
    .sort({createdAt:-1}) // first created
...
export const productCount = async(req,res)=>{
  ...
  try{
       // estimatedDocumentCount gives number product into database
...
export const listProduct = async(req,res)=>{
  ...
  try{
    ...
    const products = await Product.find({}).select("-photo").skip((page-1)*perPage).limit(6).sort({createdAt:-1})
    res.json(products);
...
export const productSearch = async (req,res)=>{
  ...
  try{
    ...
    const results = await Product.find({
      $or:[
        // regex for keyword and Options for case sensitive
